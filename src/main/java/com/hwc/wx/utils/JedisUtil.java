...
import com.hwc.wx.config.RedisConfig;
...
public class JedisUtil {
    ...
    /**缓存生存时间 */
    private final int        expire    = 12 * 60 * 60 * 1000;
    /** 操作Key的方法 */
    public Keys              KEYS;
    /** 对存储结构为String类型的操作 */
    ...
    static {
        ...
        //redis未设置了密码：
        System.out.println("redisConfig=>" + RedisConfig.host);
        if (StringUtil.isEmpty(RedisConfig.password)) {
            jedisPool = new JedisPool(config, RedisConfig.host, Integer.valueOf(RedisConfig.port),
                10000);
        } else {
            jedisPool = new JedisPool(config, RedisConfig.host, Integer.valueOf(RedisConfig.port),
                10000, RedisConfig.password);
        }
